...
void Flight_System::Update(){
  float L = (512.0*comms.Data_ManualControl.z)/125.0-(512.0*comms.Data_ManualControl.y)/625.0;
  float R = (512*comms.Data_ManualControl.z)/125.0+(512.0*comms.Data_ManualControl.y)/625.0;
  L_PWM = (unsigned int) constrain(L,0,4096);
  R_PWM = (unsigned int) constrain(R,0,4096);
  Serial.printf("MANUAL_CONTROL: ");
  Serial.printf("x= %d ",comms.Data_ManualControl.x);
  Serial.printf("y= %d ",comms.Data_ManualControl.y);
  Serial.printf("z= %d ",comms.Data_ManualControl.z);
  Serial.printf("r= %d ",comms.Data_ManualControl.r);
